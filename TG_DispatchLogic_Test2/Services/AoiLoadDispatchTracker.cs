...
/// <summary>
/// AOI 上料 in-flight：鎖定 PKP 停車點；完成條件為曾 busy 後回 idle，且車上 Port 不再全為有絲(2)。
/// </summary>
public static class AoiLoadDispatchTracker
{
    ...
    public static bool IsStationLocked(IEnumerable<AoiLoadDispatchInFlight> flights, string parkingPointId) =>
        flights.Any(f =>
            f.IsActive &&
            f.ParkingPointId.Equals(parkingPointId, StringComparison.OrdinalIgnoreCase));

    public static AoiLoadDispatchInFlight Register(
    ...
    public static List<AoiLoadDispatchInFlight> Refresh(
        IEnumerable<AoiLoadDispatchInFlight> flights,
    ...
    public static List<AoiLoadDispatchInFlight> PruneCompleted(
        IEnumerable<AoiLoadDispatchInFlight> flights) =>
        flights.Where(f => f.IsActive).ToList();

    static AoiLoadDispatchInFlight RefreshOne(
        AoiLoadDispatchInFlight flight,
        Dictionary<string, RobotStatusDto> robotMap)
    {
        ...
        if (IsRobotBusy(robot))
        {
            ...
            {
                SawRobotBusy = true,
                StatusHint = $"作業中 · {robot.State} · task={FormatTaskId(robot)} · 有絲 Port {CountFullSilk(robot)}/{GetCapacity(robot)}"
            };
        }

        if (!AreAllPortsFullSilk(robot))
        {
            return Complete(flight, true,
                $"任務完成 · 車上已卸出有絲 Bobbin（剩餘有絲 {CountFullSilk(robot)}/{GetCapacity(robot)}）");
        }
        ...
        if (elapsed.TotalSeconds >= MinBusySeconds)
        {
            ...
            {
                SawRobotBusy = true,
                StatusHint = $"車輛已回 idle，等待車上有絲 Port 減少（目前 {CountFullSilk(robot)}/{GetCapacity(robot)}）"
            };
        }
    ...
    static AoiLoadDispatchInFlight Complete(
        AoiLoadDispatchInFlight flight,
        bool sawBusy,
        string hint) =>
        flight with
    ...
    static bool AreAllPortsFullSilk(RobotStatusDto robot)
    {
        var capacity = GetCapacity(robot);
        if (robot.PortStates.Count < capacity)
            return false;
        return robot.PortStates.Take(capacity).All(v => v == 2);
    }

    static int CountFullSilk(RobotStatusDto robot) =>
        robot.PortStates.Count(v => v == 2);
